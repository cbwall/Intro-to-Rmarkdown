...
Notice the text updates to show *Figure 1* and the code chunk (displayed above) produced the figure and adds the *Figrue 1* tot he caption, despite the fact that I did not directly name it Figure 1 using `fig.cap`. This is great because now R will reference these figures chronologically and link the # to the code chunk name. If a reviewer says, "I think this should be moved to the supplement and have a new Figure 1," we can do that easily. It would require moving the order of chunks around, but that is doable. 
