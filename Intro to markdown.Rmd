...
```{r, fig.show='hold', out.width= "50%", fig.align='center', fig.cap="**Figure 2. Example of side-by-side plots at 50%, with a 'fig hold' option**"}
...
plot(density(MC.df$biomass), lwd=2, col="darksalmon", xlim=c(0,100), main="Figure 3. Tissue biomass in two coral species", xlab=expression(paste("Biomass", ~(mg~cm^-2), sep="")))
...
![**Figure 4. This is a figure of a bleached *Montipora capitata* coral during the 2014 bleaching event in Kāne'ohe Bay, O'ahu, Hawai'i. (PC: CB Wall)**](data/M cap bleached.jpg){ width=50% }
...
![**Figure 5.  Pa'āni is *beautiful* (and sassy). Beware!**](data/Paani.jpg){ width=30%}  
...
Below: This is a code chunk, and this is how you enter your data into R markdown. Note the `code chunks` always start with a ` ```{r...}` and ends with a `{...}`  
...
```{r chlorophyll a figure, results='hide', fig.align='center', fig.cap="**Figure 6. Chlorophyll a density**. *Note: This caption was set in code chunk setting*", fig.dim=c(4,4)}
...
plot(chla~Site, data=data, col="mediumseagreen",
     ylab=expression(paste("Chlorophyll", ~italic("a"+"c"[2]), ~(mu*g~cm^-2), sep="")))
