...
author: "Chris B. Wall -- cbwall@hawaii.edu"
date: "12/11/2024"
output:
  html_document:
    code_folding: hide
...
# ^^^ above: you can put global options, such as "no warnings", "no messages"
...
########## Load packages to run the analyses
...
In short, **R markdown helps make your science repeatable, accessible, and data analysis coherent!**  
  
Here are some common commands and examples of what Markdown can do for you. I recommend visiting the [R Bookdown for a complete and exhaustive guide](https://bookdown.org/yihui/rmarkdown/) or the [R markdown cheat sheet](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf)
...
Is this all above your head? That's okay. See some open access resources here to help you get started in R (notice the pages are all R-Markdown html objects!) and on the path to coding. [R data carpentry course](https://datacarpentry.org/R-ecology-lesson/index.html).
...
In R Studio you can run a Project from your local directory or make one with version control by clicking the option to make an R project with version control will set you on the path to linking with GitHub (if you go this route, see [happygitwithR.com](http://happygitwithr.com/rstudio-git-github.html) first, as you must set up a github account, then make a repository, then use the repository URL to link to your R project). There are a few more steps to integrating R and github now, since github has moved to two-factor authentication, but you can see the [how to](https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/accessing-github-using-two-factor-authentication) and some [trouble shooting](https://stackoverflow.com/questions/66065099/how-to-update-github-authentification-token-on-rstudio-to-match-the-new-policy) in the linked sites. 
...
![**Figure 1. Data Directory example**](data/directory example.png){ width=30%}
...
# set factor levels
facs <- c("Time.point", "Period", "Site", "Species", "Status", "Sample.ID")
...
# use 'data', loaded above

ggplot(data, aes(y=biomass, x=Site, fill=Status))+
  geom_boxplot() +
  ylab(expression(paste("Biomass", ~(mg~cm^-2), sep="")))+
  scale_fill_manual(values=c("lightblue", "mediumseagreen"))+
  theme_classic()

ggplot(data, aes(y=chla, x=Site, fill=Status))+
  geom_boxplot() +
  ylab(expression(paste("Chlorophyll", ~italic("a"+"c"[2]), ~(mu*g~cm^-2), sep="")))+
  scale_fill_manual(values=c("lightsalmon", "maroon"))+
  theme_classic()
...
dev.print(pdf, "figures/biomass in two species.pdf", height=4, width=6) # export in directory
dev.off()
...
# sometimes loading the repo .csv gives an error message here, not sure why! Somehow the factor "Status" gets changed to integer incorrectly, should be fixed above in ~ line 190. In case, not, set it again.
...
anova(mod, type=3) # fixed effects
